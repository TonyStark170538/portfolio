...
const GLOBAL_CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  body { background: ${C.bg}; color: ${C.platinum}; font-family: 'Space Mono', monospace; }
...
function Chip({ children, color, style }: { children: React.ReactNode; color?: string; style?: React.CSSProperties }) {
  ...
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '2px 8px', borderRadius: 2, background: c + '18', border: `1px solid ${c}44`, color: c, fontSize: 10, fontWeight: 700, letterSpacing: 1, ...style }}>
      {children}
    </span>
  )
}
...
function GhostBtn({ onClick, disabled, children, active, style }: { onClick?: () => void; disabled?: boolean; children: React.ReactNode; active?: boolean; style?: React.CSSProperties }) {
  return (
    <button onClick={onClick} disabled={disabled} style={{
      ...
      borderRadius: 2, color: active ? C.blue : C.muted,
      cursor: disabled ? 'not-allowed' : 'pointer',
      fontFamily: "'Space Mono', monospace", fontSize: 10,
      fontWeight: 700, letterSpacing: 1, padding: '5px 12px',
      transition: 'all 0.15s', textTransform: 'uppercase', opacity: disabled ? 0.4 : 1,
      ...style,
    }}>{children}</button>
  )
}
...
function PrimaryBtn({ onClick, disabled, children }: { onClick?: () => void; disabled?: boolean; children: React.ReactNode }) {
  return (
    <button onClick={onClick} disabled={disabled} style={{
      background: disabled ? C.faint : C.blue,
      border: 'none', borderRadius: 2,
      color: disabled ? C.muted : '#fff',
      cursor: disabled ? 'not-allowed' : 'pointer',
      fontFamily: "'Syne', sans-serif", fontSize: 12,
      fontWeight: 700, letterSpacing: 2, padding: '11px 28px',
      transition: 'all 0.2s', textTransform: 'uppercase',
      boxShadow: disabled ? 'none' : `0 0 20px ${C.blueGlow}`,
    }}>{children}</button>
  )
}
...
function StatCard({ label, value, color, sub }: { label: string; value: string; color?: string; sub?: string }) {
  return (
    <div style={{ padding: '14px 18px', background: C.card, border: `1px solid ${C.border}`, borderRadius: 3, minWidth: 120 }}>
      <Label style={{ marginBottom: 8 }}>{label}</Label>
      <div style={{ fontFamily: "'Syne', sans-serif", fontSize: 22, fontWeight: 800, color: color ?? C.white, lineHeight: 1, letterSpacing: -0.5 }}>{value}</div>
      {sub && <div style={{ fontSize: 9, color: C.muted, marginTop: 5, lineHeight: 1.4 }}>{sub}</div>}
    </div>
  )
}
...
function MarketBriefCard({ brief, loading, error, onFetch, hasAssets }: { brief: MarketBrief | null; loading: boolean; error: string | null; onFetch: () => void; hasAssets: boolean }) {
  ...
  return (
    <div style={{ border: `1px solid ${C.border}`, borderRadius: 3, overflow: 'hidden', marginTop: 8 }}>
      {/* Header */}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 16px', background: C.faint, borderBottom: `1px solid ${C.border}` }}>
      ...
      <div style={{ padding: 16 }}>
        {!brief && !loading && !error && (
          <p style={{ color: C.muted, fontSize: 11, fontStyle: 'italic', lineHeight: 1.6 }}>
            {hasAssets ? 'Search the web for current conditions and get personalised advice.' : 'Add assets first.'}
          </p>
        )}
        {loading && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '8px 0' }}>
            <div style={{ display: 'flex', gap: 5 }}>
              {[0,1,2,3].map(i => <div key={i} style={{ width: 5, height: 5, borderRadius: '50%', background: C.blue, animation: `pulse 1.4s ease-in-out ${i * 0.18}s infinite` }} />)}
            </div>
        ...
        {brief && !loading && (
          <div className="anim-in" style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div>
              <Label style={{ marginBottom: 6 }}>What's happening</Label>
              <p style={{ fontSize: 12, color: C.platinum, lineHeight: 1.8 }}>{brief.summary}</p>
            </div>
            {brief.outlook && (
              <div>
                <Label style={{ marginBottom: 6 }}>Portfolio Outlook</Label>
                <p style={{ fontSize: 12, color: C.platinum, lineHeight: 1.8 }}>{brief.outlook}</p>
              </div>
            )}
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {brief.advice.map((tip, i) => (
                  <div key={i} style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
                    <div style={{ flexShrink: 0, width: 18, height: 18, background: C.blue, color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 9, fontWeight: 700, borderRadius: 1 }}>{i + 1}</div>
            ...
            <div style={{ borderTop: `1px solid ${C.border}`, paddingTop: 10 }}>
              <p style={{ fontSize: 9, color: C.muted, letterSpacing: 0.5 }}>AI-GENERATED COMMENTARY · NOT FINANCIAL ADVICE · ALWAYS DO YOUR OWN RESEARCH</p>
            </div>
          </div>
        )}
      </div>
    ...
    <div className="anim-in" style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      <div>
        ...
        <div style={{ border: `1px solid ${C.border}`, borderRadius: 3, overflow: 'hidden' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr repeat(3,120px)', padding: '8px 16px', background: C.faint, borderBottom: `1px solid ${C.border}` }}>
            {['Asset','Before','After','Change'].map(h => <Label key={h}>{h}</Label>)}
          </div>
          {result.assets.map((a, i) => {
            const diff = a.new - a.old
            return (
              <div key={i} style={{ display: 'grid', gridTemplateColumns: '1fr repeat(3,120px)', padding: '12px 16px', borderBottom: i < result.assets.length - 1 ? `1px solid ${C.border}` : 'none', alignItems: 'center' }}>
                <span style={{ fontSize: 12, color: C.platinum }}>{a.name}</span>
                <span style={{ fontSize: 12, color: C.muted }}>{fmt(a.old)}</span>
                <span style={{ fontSize: 12, color: C.platinum }}>{fmt(a.new)}</span>
                <span style={{ fontSize: 12, color: diff < 0 ? C.red : C.green, fontWeight: 700 }}>{diff < 0 ? '−' : '+'}  {fmt(Math.abs(diff))}</span>
              </div>
            )
          })}
        </div>
...
function HistoryChart({ assets }: { assets: HistoryAsset[] }) {
  if (assets.length === 0) return (
    <div style={{ color: C.muted, fontSize: 11, textAlign: 'center', padding: '40px 0', border: `1px dashed ${C.border}`, borderRadius: 3 }}>
  ...
  const W = 800, H = 280, PAD = { top: 20, right: 90, bottom: 36, left: 56 }
  ...
  const pad = (Math.max(...allY) - Math.min(...allY)) * 0.12
  ...
  function path(vals: number[], startIdx: number) {
    return vals.map((v, i) => `${i === 0 ? 'M' : 'L'}${xS(startIdx + i).toFixed(1)},${yS(v).toFixed(1)}`).join(' ')
  }

  const firstDate = assets[0]?.dates[0]
  const midDate = assets[0]?.dates[Math.floor(assets[0].dates.length / 2)]
  const xLabels = [
  ...
  const yLabels = Array.from({ length: yTicks }, (_, i) => {
    ...
    return { y: yS(v), label: (v >= 0 ? '+' : '') + v.toFixed(1) + '%' }
  })
  ...
  return (
    <div style={{ overflowX: 'auto' }}>
      <svg width={W} height={H} style={{ display: 'block' }}>
        ...
        <text x={todayX + 5} y={PAD.top + 11} fill={C.blue} fontSize={8} fontFamily="'Syne',sans-serif" fontWeight={700} letterSpacing={1}>TODAY</text>

        {assets.map((a, ai) => {
          const color = ASSET_COLORS[ai % ASSET_COLORS.length]
          const simStart = a.closes.length - 1
          const lastClose = a.closes[simStart]
          const offset = (v: number) => lastClose + v
          const bestFan = a.fan.best.map(offset)
          const medFan = a.fan.median.map(offset)
          const worstFan = a.fan.worst.map(offset)

          return (
            <g key={ai}>
              {/* Fan fill */}
              <path d={`${path(bestFan, simStart)} ${[...worstFan].reverse().map((v, i) => `L${xS(simStart + worstFan.length - 1 - i).toFixed(1)},${yS(v).toFixed(1)}`).join(' ')} Z`} fill={color} fillOpacity={0.05} />
              <path d={path(worstFan, simStart)} fill="none" stroke={C.red} strokeWidth={1.5} strokeDasharray="4 3" opacity={0.65} />
              <path d={path(bestFan, simStart)} fill="none" stroke={C.green} strokeWidth={1.5} strokeDasharray="4 3" opacity={0.65} />
              <path d={path(medFan, simStart)} fill="none" stroke={C.amber} strokeWidth={2} strokeDasharray="6 3" opacity={0.8} />
              <path d={path(a.closes, 0)} fill="none" stroke={color} strokeWidth={2} />
              <circle cx={xS(simStart)} cy={yS(lastClose)} r={4} fill={C.bg} stroke={color} strokeWidth={2} />
            </g>
          )
        })}

        {/* Y labels */}
        {yLabels.map((l, i) => <text key={i} x={PAD.left - 6} y={l.y + 4} fill={C.muted} fontSize={8} fontFamily="'Space Mono',monospace" textAnchor="end">{l.label}</text>)}

        {/* X labels */}
        {xLabels.map((l, i) => <text key={i} x={l.x} y={H - PAD.bottom + 16} fill={(l as {hi?: boolean}).hi ? C.blue : C.muted} fontSize={8} fontFamily="'Syne',sans-serif" fontWeight={700} letterSpacing={1} textAnchor="middle">{l.label}</text>)}
      </svg>

      {/* Legend */}
      <div style={{ display: 'flex', gap: 20, flexWrap: 'wrap', marginTop: 12, paddingLeft: PAD.left }}>
        {assets.map((a, i) => (
          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
            <div style={{ width: 20, height: 2, background: ASSET_COLORS[i % ASSET_COLORS.length] }} />
            <span style={{ fontSize: 10, color: C.platinum }}>{a.name}</span>
            <Chip color={ASSET_COLORS[i % ASSET_COLORS.length]} style={{ fontSize: 9 }}>{a.ticker}</Chip>
          </div>
        ))}
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginLeft: 4 }}>
          {[{ color: C.green, label: 'Best P90' }, { color: C.amber, label: 'Median' }, { color: C.red, label: 'Worst P10' }].map(l => (
            <div key={l.label} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <div style={{ width: 16, height: 2, background: l.color, opacity: 0.8 }} />
              <span style={{ fontSize: 9, color: l.color, fontWeight: 700, letterSpacing: 0.5 }}>{l.label}</span>
            </div>
          ))}
        </div>
      </div>
...
function RiskPanel({ result, fmt, assets }: { result: RiskResult; fmt: (n: number) => string; assets: Asset[] }) {
  ...
  const hist = Array.from({ length: buckets }, (_, i) => {
    ...
    return mc.results.filter(r => r >= lo && (i === buckets - 1 ? r <= hi : r < hi)).length
  })
  const histMax = Math.max(...hist)
  const anyVol = heatmap.some(h => h.vol_available)

  return (
    <div className="anim-in" style={{ display: 'flex', flexDirection: 'column', gap: 28 }}>
      {/* Stats */}
      <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
        ...
        <StatCard label="95% VaR" value={fmt(v.var_95)} color={C.red} sub="Max loss at 95% confidence" />
        <StatCard label="99% VaR" value={fmt(v.var_99)} color={C.red} sub="Max loss at 99% confidence" />
      </div>

      {/* Historical chart */}
      <div>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
          <Label>Historical Performance + Simulation Forecast</Label>
          <GhostBtn onClick={fetchHistory} disabled={histLoading} active={!histFetched}>
            {histLoading ? '⟳ Loading' : histFetched ? '↻ Refresh' : '▶ Load 1Y Chart'}
          </GhostBtn>
        </div>
        {histError && <div style={{ color: C.red, fontSize: 11, marginBottom: 8 }}>⚠ {histError}</div>}
        {histLoading && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '20px 0', color: C.muted, fontSize: 11 }}>
            <div style={{ display: 'flex', gap: 4 }}>
              {[0,1,2].map(i => <div key={i} style={{ width: 4, height: 4, borderRadius: '50%', background: C.blue, animation: `pulse 1.2s ease-in-out ${i * 0.2}s infinite` }} />)}
            </div>
            Fetching 1 year of price data from Yahoo Finance…
          </div>
        )}
        {!histLoading && histFetched && <HistoryChart assets={historyData} />}
        {!histLoading && !histFetched && (
          <div style={{ padding: '20px 0', color: C.muted, fontSize: 11 }}>
            Click <span style={{ color: C.blue }}>Load 1Y Chart</span> to fetch real price history for assets with tickers.
          </div>
        )}
      </div>
      ...
      {/* Histogram */}
      <div>
        <Label style={{ marginBottom: 14 }}>Outcome Distribution — 500 Simulations · 50 Days</Label>
        <div style={{ display: 'flex', alignItems: 'flex-end', gap: 3, height: 72 }}>
          {hist.map((count, i) => {
            const ratio = count / histMax
            const mid = buckets / 2
            const color = i < mid * 0.4 ? C.red : i > mid * 1.6 ? C.green : C.blue
            return (
              <div key={i} title={`${count} simulations`} style={{
                flex: 1, height: `${ratio * 100}%`, minHeight: 2,
                background: color, opacity: 0.4 + ratio * 0.5,
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 6 }}>
          <span style={{ fontSize: 9, color: C.red }}>↓ {fmt(min)} worst</span>
          <span style={{ fontSize: 9, color: C.muted }}>range of outcomes</span>
          <span style={{ fontSize: 9, color: C.green }}>↑ {fmt(max)} best</span>
        </div>
      </div>
      ...
      <div>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
          ...
          {anyVol
            ? <span style={{ fontSize: 9, color: C.muted, letterSpacing: 1 }}>COMPOSITE = 50% VOLATILITY + 50% CONCENTRATION</span>
            : <Chip color={C.amber}>⚠ Add tickers for live volatility</Chip>}
        </div>
        <div style={{ border: `1px solid ${C.border}`, borderRadius: 3, overflow: 'hidden' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 70px 80px 80px 1fr', padding: '8px 16px', background: C.faint, borderBottom: `1px solid ${C.border}`, gap: 12 }}>
            {['Asset','Weight','Ann. Vol','Score','Risk Bar'].map(h => <Label key={h}>{h}</Label>)}
          </div>
          {heatmap.map((h, i) => {
            const color = RISK_COLOR(h.risk_score)
            return (
              <div key={i} style={{ display: 'grid', gridTemplateColumns: '1fr 70px 80px 80px 1fr', padding: '12px 16px', borderBottom: i < heatmap.length - 1 ? `1px solid ${C.border}` : 'none', gap: 12, alignItems: 'center' }}>
                <div>
                ...
                <div style={{ fontSize: 12, color: C.muted, textAlign: 'right' }}>{h.weight}%</div>
                <div style={{ fontSize: 12, textAlign: 'right' }}>
                  {h.vol_available ? <span style={{ color }}>{h.volatility_pct}%</span> : <span style={{ color: C.faint }}>—</span>}
                </div>
                <div style={{ textAlign: 'right' }}>
                  <span style={{ fontFamily: "'Syne',sans-serif", fontSize: 16, fontWeight: 800, color }}>{h.risk_score}</span>
...
export default function Page() {
  ...
  const TABS = [
    ...
    { id: 'risk' as Tab,     label: 'Risk Engine',  icon: '△' },
  ]
  ...
  return (
    <>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', background: C.bg }}>
        ...
        {/* Header */}
        <header style={{ borderBottom: `1px solid ${C.border}`, padding: '0 28px', display: 'flex', alignItems: 'center', height: 56, gap: 16, flexShrink: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <div>
              <div style={{ fontFamily: "'Syne',sans-serif", fontSize: 14, fontWeight: 800, color: C.blue, letterSpacing: 2 }}>PORTFOLIO</div>
              <div style={{ fontSize: 8, color: C.white, letterSpacing: 2 }}>AI TERMINAL</div>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <div>
              <div style={{ fontSize: 8, color: C.muted, letterSpacing: 1, textAlign: 'right' }}>RATES</div>
            ...
            {(['USD', 'SEK', 'EUR'] as Currency[]).map(c => {
              ...
              return (
                <button key={c} onClick={() => setCurrency(c)} style={{
                  ...
                  borderRadius: 2, color: currency === c ? '#fff' : C.muted,
                  cursor: 'pointer', fontFamily: "'Syne',sans-serif", fontSize: 9,
                  fontWeight: 700, letterSpacing: 1, padding: '4px 9px', transition: 'all 0.15s',
                }}>{flag} {c}</button>
              )
            })}
          </div>
        ...
        <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
          ...
          <aside style={{ width: 268, flexShrink: 0, borderRight: `1px solid ${C.border}`, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            <div style={{ padding: '16px 18px', borderBottom: `1px solid ${C.border}` }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                <input value={name} onChange={e => setName(e.target.value)} onKeyDown={e => e.key === 'Enter' && addAsset()} placeholder="Asset name" style={iStyle} />
                <input value={value} onChange={e => setValue(e.target.value)} onKeyDown={e => e.key === 'Enter' && addAsset()} placeholder={`Value in ${currency}`} type="number" min={0} style={iStyle} />
                <div style={{ position: 'relative' }}>
                  <input value={ticker} onChange={e => setTicker(e.target.value.toUpperCase())} onKeyDown={e => e.key === 'Enter' && addAsset()} placeholder="Ticker (AAPL, BTC-USD…)" style={{ ...iStyle, paddingRight: 56 }} />
                ...
                <GhostBtn onClick={addAsset} active style={{ width: '100%', padding: '8px', fontSize: 10 }}>+ Add Asset</GhostBtn>
              </div>
            </div>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '12px 18px' }}>
              {assets.length === 0 ? (
                <p style={{ fontSize: 10, color: C.faint, textAlign: 'center', marginTop: 24, lineHeight: 1.6 }}>Add your first asset<br />to get started</p>
              ) : (
                <>
                  ...
                  {assets.map((a, i) => (
                    <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '9px 0', borderBottom: `1px solid ${C.border}` }}>
                      ...
                      <button onClick={() => removeAsset(i)} style={{ background: 'none', border: 'none', color: C.faint, cursor: 'pointer', fontSize: 14, padding: '0 4px', flexShrink: 0, transition: 'color .15s' }}
            ...
            <div style={{ display: 'flex', borderBottom: `1px solid ${C.border}`, padding: '0 28px', flexShrink: 0 }}>
              {TABS.map(t => (
                <button key={t.id} onClick={() => setTab(t.id)} style={{
                  ...
                  <span style={{ color: tab === t.id ? C.blue : C.faint }}>{t.icon}</span>
                  {t.label}
                </button>
              ))}
            </div>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: 28, display: 'flex', flexDirection: 'column', gap: 20 }}>
                ...
                <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                  ...
                  {SCENARIOS.map(s => (
                    <button key={s.id} onClick={() => setScenario(s.id)} style={{
                      ...
                      borderRadius: 2, color: scenario === s.id ? s.color : C.muted,
              ...
              {tab === 'risk' && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 14px', background: C.green + '08', border: `1px solid ${C.green}22`, borderRadius: 2 }}>
                  ...
                  <span style={{ fontSize: 10, color: C.muted }}>Fetches live price history from Yahoo Finance for assets with tickers. May take a few seconds.</span>
                </div>
              )}

              {/* Run button */}
              <div>
                <PrimaryBtn onClick={run} disabled={loading || assets.length === 0}>
              ...
              <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 3, padding: 24, minHeight: 180 }}>
                {!activeResult && !loading && (
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: 140, gap: 8 }}>
                    <div style={{ fontSize: 32, color: C.faint }}>◇</div>
                    <p style={{ fontSize: 11, color: C.muted }}>
                      {assets.length === 0 ? 'Add assets in the sidebar to get started.' : 'Press Run to generate results.'}
                    </p>
                  </div>
                )}
                {loading && (
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: 140, gap: 12 }}>
                    <div style={{ display: 'flex', gap: 6 }}>
                      {[0,1,2,3].map(i => <div key={i} style={{ width: 6, height: 6, borderRadius: '50%', background: C.blue, animation: `pulse 1.4s ease-in-out ${i*0.18}s infinite` }} />)}
                    </div>
